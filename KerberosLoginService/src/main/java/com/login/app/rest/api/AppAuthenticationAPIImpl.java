...
public class AppAuthenticationAPIImpl  implements IAppAuthenticationAPI{
	...
	@Override
	public AppAuthenticationResponse createAppAuthenticationResponse(AppSession appSession, Date requestAuthenticator, String path, SecretKey serviceSessionKey)  {
		...
		String[] encryptedData = iEncryptionUtil.encrypt(serviceSessionKey, appSession.getSessionID(),  iDateUtil.generateStringFromDate(responseAuthenticator), 
				iDateUtil.generateStringFromDate(appSession.getExpiryTime()));
		...
		appSession.createRequest(path, requestAuthenticator).createResponse(200, responseAuthenticator);
